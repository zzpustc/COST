...
from methods.teleportation import teleporation, model_diff
...
class WeightMethod:
    ...
    def backward(
        ...
        train_loader,
        lora_config,
        cur_data, # = [train_data, train_depth, train_label],
        epoch,
        optimizer,
        schedule,
        ...
        **kwargs,
        ...
        Loss, extra outputs
        """
        loss, extra_outputs, optimizer = self.get_weighted_loss(
            losses=losses,
            opt = opt,
        ...
        if loss >= 0:
            loss.backward()
        return loss, extra_outputs, optimizer
    ...
    def tele_fun(self, opt, multi_task_model, lora_loader, lora_config, cur_data, loss_backup, ratio_list, **kwargs):
        teleporation(opt, multi_task_model, lora_loader, lora_config, cur_data, loss_backup, ratio_list, **kwargs)
...
class CAGrad(WeightMethod):
    def __init__(self, n_tasks, device: torch.device, c=2.0, max_norm=1.0):
        super().__init__(n_tasks, device=device)
        self.alpha = c
        self.max_norm = max_norm
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        g, GTG, w_cpu = self.fairgrad(grads, losses, alpha=self.alpha)
        self.overwrite_grad(shared_parameters, g, grad_dims)
        return GTG, w_cpu

    def mgd(self, grads, losses):
    ...
    def fairgrad(self, grads, losses, alpha=1.0):
        GG = grads.t().mm(grads).cpu()  # [num_tasks, num_tasks]

        g_0, g_m = grads.mean(1).unsqueeze(0), self.mgd(grads, losses)

        cos_init = F.cosine_similarity(g_0, g_m)
        cos_value = cos_init.item()
        cos_max = cos_value
        ...
        def objfn(x):
            ...
            return (1.0 - cos_max) * np.dot(A, x) - cos_max * np.power(1 / x, 1 / alpha)
    ...
    def backward(
        ...
        **kwargs,
        ...
        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return np.ones(40), {"GTG": GTG, "weights": w}, None  # NOTE: to align with all other weight methods
...
class FairGrad(WeightMethod):
    ...
    def get_weighted_loss(
        ...
        **kwargs,
        ...
        if not kwargs['opt'].tele_sign:
            g, GTG, w_cpu = self.fairgrad(grads, alpha=self.alpha)
            self.overwrite_grad(shared_parameters, g, grad_dims)
            return GTG, w_cpu, optimizer
        
        # for teleportation
        G0 = torch.mean(grads, 1).unsqueeze(0)
        norm_list = np.array([torch.norm(grads[:, i].unsqueeze(0)).item() for i in range(self.n_tasks)])
        ratio_list = self.n_tasks * np.array([1 - norm_list[i] / np.sum(norm_list) for i in range(self.n_tasks)])
        sim_list = [F.cosine_similarity(G0, grads[:, i].unsqueeze(0)) for i in range(self.n_tasks)]
           
        if min(sim_list) <= 0.0 and kwargs['epoch'] >= kwargs['opt'].lora_epoch and self.tele_count < kwargs['opt'].tele_times_per_epoch:
            multi_task_model_4_diff = deepcopy(kwargs['multi_task_model'])
            loss_backup = [losses[ii].item() for ii in range(self.n_tasks)]
            self.tele_fun(kwargs['opt'], kwargs['multi_task_model'], kwargs['lora_loader'], kwargs['lora_config'], kwargs['cur_data'], loss_backup, ratio_list)
            self.tele_count += 1

            # reset optimizer / historical trajectory reuse 
            with torch.no_grad():
                grads_diff = model_diff(multi_task_model_4_diff, kwargs['multi_task_model'])
                grad_sim = torch.cosine_similarity(self.g_former, grads_diff.unsqueeze(0))
                print('Grad Sim:', grad_sim.item())
                if grad_sim <= 0:
                    del optimizer
                    optimizer = torch.optim.Adam(
                    [
                    dict(params=kwargs['multi_task_model'].parameters(), lr=kwargs['opt'].lr),
                    dict(params=self.parameters(), lr=kwargs['opt'].method_params_lr),
                    ],
                    )
                else:
                    print('-----Historical trajectory reuse-----')
                    reset_beta1, reset_beta2 = 0.9 * grad_sim.item(), 0.999 * grad_sim.item()
                    kwargs['optimizer'].param_groups[0]['betas'] = [reset_beta1, reset_beta2]
                    self.adam_sign = False

            return G0, np.zeros(self.n_tasks), optimizer

        else:
            g, GTG, w_cpu = self.fairgrad(grads, alpha=self.alpha)
            self.g_former = g
            self.overwrite_grad(shared_parameters, g, grad_dims)
            if not self.adam_sign:
                kwargs['optimizer'].param_groups[0]['betas'] = [0.9, 0.999]
                self.adam_sign = True
        
            return GTG, w_cpu, kwargs['optimizer']
    ...
    def backward(
        ...
        **kwargs,
        ...
        if kwargs['opt'].tele_sign:
            target_modules = kwargs['multi_task_model'].get_lora_target_modules()
            lora_config = peft.LoraConfig(r=5, target_modules=target_modules)
            kws = {"opt": kwargs['opt'], "multi_task_model":kwargs['multi_task_model'],"lora_loader":kwargs['train_loader'], 'cur_data':kwargs['cur_data'], 'epoch':kwargs['epoch'], 'optimizer': kwargs['optimizer'], 'lora_config': lora_config}
            GTG, w, optimizer = self.get_weighted_loss(losses, shared_parameters, **kws)
        else:
            GTG, w, optimizer = self.get_weighted_loss(losses, shared_parameters)

        if self.max_norm > 0:
            torch.nn.utils.clip_grad_norm_(shared_parameters, self.max_norm)
        return w, {"GTG": GTG, "weights": w}, optimizer  # NOTE: to align with all other weight methods
