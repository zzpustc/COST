...
def main(path, lr, bs, device):
    ...
    lora_loader = deepcopy(train_loader)

    for epoch in epoch_iter:
        cost = np.zeros(24, dtype=np.float32)
        weight_method.method.tele_sign = True
        for j, batch in enumerate(train_loader):
            ...
            loss, extra_outputs, optimizer = weight_method.backward(
                losses=losses,
                opt = args,
                multi_task_model = model,
                train_loader = lora_loader,
                cur_data = [train_data, train_label, train_depth, train_normal],
                epoch = epoch,
                optimizer = optimizer,
    ...
    print('Epoch: {:04d} | TRAIN: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} ||'
            'TEST: {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} | {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} {:.4f} | delta_m: {:.4f}'
            .format(epoch, np.mean(avg_cost[-10:, 0]), np.mean(avg_cost[-10:, 1]), np.mean(avg_cost[-10:, 2]), np.mean(avg_cost[-10:, 3]),
                    np.mean(avg_cost[-10:, 4]), np.mean(avg_cost[-10:, 5]), np.mean(avg_cost[-10:, 6]), np.mean(avg_cost[-10:, 7]), np.mean(avg_cost[-10:, 8]),
                    np.mean(avg_cost[-10:, 9]), np.mean(avg_cost[-10:, 10]), np.mean(avg_cost[-10:, 11]), np.mean(avg_cost[-10:, 12]), np.mean(avg_cost[-10:, 13]),
                    np.mean(avg_cost[-10:, 14]), np.mean(avg_cost[-10:, 15]), np.mean(avg_cost[-10:, 16]), np.mean(avg_cost[-10:, 17]), np.mean(avg_cost[-10:, 18]),
                    np.mean(avg_cost[-10:, 19]), np.mean(avg_cost[-10:, 20]), np.mean(avg_cost[-10:, 21]), np.mean(avg_cost[-10:, 22]), np.mean(avg_cost[-10:, 23]), np.mean(deltas[-10:])))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_epoch", type=int, default=0, help="Epoch when start teleportation")
