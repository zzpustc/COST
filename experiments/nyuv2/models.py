...
class SegNetMtan(nn.Module):
    ...
    def get_lora_target_modules(self):
        ...
        target_module = []
        all_modules = [n for n,p in self.segnet.named_parameters()]
        shared_modules = self.shared_modules_names()
        for nn in all_modules:
            for qq in shared_modules:
                # pdb.set_trace()
                if qq in nn and '.0.weight' in nn:
                    target_module.append(nn[:-7])
                    break

        return target_module

    def disable_grad(self):
        for n, p in self.segnet.named_parameters():
