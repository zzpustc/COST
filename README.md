...
### Run Experiment
The dataset by default should be put under `experiments/EXP_NAME/dataset/` folder where `EXP_NAME` is chosen from `{celeba, cityscapes, nyuv2, quantum_chemistry}`. Currently, this implementation is based on FairGrad. To run the experiment:
```
cd cityscapes/
sh run.sh
```
